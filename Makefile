...
build-like-porter: login
	docker build --pull -t $(BUILDER_TAG) --build-arg ARTIFACTORY_USER --build-arg ARTIFACTORY_API_TOKEN -f $(BUILD_DOCKERFILE_NAME) .
	docker run $(BUILDER_TAG) | docker build --pull --tag $(IMAGE_TAG) -

build: login
	# A separate image for build allows the process to avoid dependencies with the build machine.
	docker build --pull -t $(BUILDER_TAG) -f $(BUILD_DOCKERFILE_NAME) .
